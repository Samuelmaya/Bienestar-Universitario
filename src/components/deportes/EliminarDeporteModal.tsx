...
import { eliminarDeporte } from "@/shared/services/deportes.service";
...
export function EliminarDeporteModal({ deporte, triggerElement, onClose, onDeleted }: Props) {
  ...
  const onConfirm = async () => {
    ...
    setError("");
    try {
      await eliminarDeporte(deporte.cod_deporte);
      onDeleted(deporte.cod_deporte);
    } catch (err) {
      setError(err instanceof Error ? err.message : "Error eliminando deporte");
    } finally {
      setLoading(false);
    }
  };
