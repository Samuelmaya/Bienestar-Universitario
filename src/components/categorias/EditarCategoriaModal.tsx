...
import { actualizarCategoria } from "@/shared/services/categorias.service";

type Props = {
  categoria: CategoriaArticulo;
  triggerElement: HTMLElement | null;
  onClose: () => void;
  onUpdated: (categoria: CategoriaArticulo) => void;
};

export function EditarCategoriaModal({ categoria, triggerElement, onClose, onUpdated }: Props) {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.nombre.trim()) {
      setError("Completa el nombre de la categoria.");
      return;
    }
    setLoading(true);
    setError("");
    try {
      const updated = await actualizarCategoria(categoria.id_categoria, {
        nombre: form.nombre.trim(),
        descripcion: form.descripcion?.trim() || null,
      });
      onUpdated(updated);
    } catch (err) {
      setError(err instanceof Error ? err.message : "Error actualizando categoria");
    } finally {
      setLoading(false);
    }
  };
    ...
    >
      <div className="w-full max-w-[420px] mx-auto pt-6 pb-10">
        ...
        >
          ...
          >
            {loading ? "Actualizando..." : "Actualizar categoria"}
          </button>
        </form>
      </div>
    </ReusableModal>
