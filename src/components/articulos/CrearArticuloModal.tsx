...
import type { ArticuloDeportivo, ArticuloCreateRequest } from "@/shared/dtos/articulo.dto";
import type { CategoriaArticulo } from "@/shared/dtos/categoria.dto";
import { crearArticulo } from "@/shared/services/articulos.service";
import { listarCategorias } from "@/shared/services/categorias.service";

type Props = {
  triggerElement: HTMLElement | null;
  onClose: () => void;
  onCreated: (articulo: ArticuloDeportivo) => void;
};

export function CrearArticuloModal({ triggerElement, onClose, onCreated }: Props) {
  const [form, setForm] = useState<ArticuloCreateRequest>({
    nombre: "",
    cantidad: 0,
    dañados: 0,
    estado: "disponible",
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    setError("");
    try {
      const created = await crearArticulo({
        ...form,
      ...
      setError(err instanceof Error ? err.message : "Error creando articulo");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    ...
    >
      <div className="w-full max-w-[440px] mx-auto pt-6 pb-10">
        ...
        >
          Registra un nuevo articulo en inventario.
        </p>
