...
import { eliminarArticulo } from "@/shared/services/articulos.service";
...
export function EliminarArticuloModal({ articulo, triggerElement, onClose, onDeleted }: Props) {
  ...
  const onConfirm = async () => {
    ...
    try {
      ...
      onDeleted(articulo.id_articulo);
    } catch (err) {
      setError(err instanceof Error ? err.message : "Error eliminando articulo");
    } finally {
      setLoading(false);
    }
  };
