...
import { eliminarMedalla } from "@/services/medal.service";
...
export function EliminarMedallaModal({ medalla, triggerElement, onClose, onDeleted }: Props) {
  ...
  const onConfirm = async () => {
    ...
    try {
      await eliminarMedalla(medalla.id_medalla);
      onDeleted();
      onClose();
    } catch (err) {
      setError(err instanceof Error ? err.message : "Error eliminando medalla");
    } finally {
      setLoading(false);
    }
  };
    ...
    >
      <div className="w-full max-w-[360px] mx-auto pt-6 pb-10">
        ...
        <div
          ...
          style={{ backgroundColor: "rgba(163,65,61,0.08)" }}
        >
          <p className="text-sm font-semibold" style={{ color: "#3b1411" }}>
            {medalla.nombre_estudiante}
          </p>
          <p className="text-xs" style={{ color: "#7a4b49" }}>{medalla.disciplina}</p>
          <p className="text-xs mt-1" style={{ color: "#7a4b49" }}>{medalla.tipo_medalla}</p>
        </div>

        {error && (
          <p className="text-xs mb-3" style={{ color: "#c2185b" }}>{error}</p>
        )}
        ...
        <div className="flex gap-2">
          ...
          <button
            ...
            style={{ backgroundColor: "#c0392b", fontFamily: "'DM Sans', system-ui, sans-serif" }}
          >
